...
namespace SensorsAndSuch.Maps
{
    ...
    {        
        ...
        private static int mapHeight = RoomHeight * 4;
        ...
        public int CreateMap()
        {
            if (state <= 0)
            {
                ...
                elementals = new BaseCrawler[10];

                BaseCrawler.AvgLife = 100;
                state--;
                if (-1 * state >= MapWidth)
                    state = 1;
                return state;
            ...
            {
                //AddBorder(50);
                regions = new ForestRegion[mapWidth / RoomWidth - 1, mapHeight / RoomHeight - 1];
                for (int i = 0; i < mapWidth / RoomWidth -1; i++)
                    for (int j = 0; j < mapHeight / RoomHeight -1; j++)
                        regions[i, j] = new ForestRegion(X: i * RoomWidth + 10, Y: j * RoomHeight);
            }
            else if (state <10)
            {
                for (int i = 0; i < mapWidth / RoomWidth -1; i++)
            ...
            {
                /*while (false && GetNumbAtHeight(1) > 20 * 25)
                {
                    while (GetNumbAtHeight(1) > 20 * 25)
                        RemoveRandPoint();
                    SetTilesToNotHit();
                    freePos = GetRandomFreePos(height: 1);
                    HitAttatchedTiles((int)freePos.X, (int)freePos.Y);
                    AddTheNotHit();

                }*/

                freePos = GetRandomFreePos(tileList => tileList.Count == 1 && tileList[0].hit == false);
                HitAttatchedTiles((int)freePos.X, (int)freePos.Y);
                //AddTheNotHit();
        ...
        //private static int RoomWidth = 14;
        //private static int RoomHeight = 14;
