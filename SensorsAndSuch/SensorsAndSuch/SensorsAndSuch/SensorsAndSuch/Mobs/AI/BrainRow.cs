...
namespace SensorsAndSuch.Mobs.AI
{
    internal class BrainRow
    {
        ...
        protected Func<float, float> sigmoid = (input => (float) ( 1 / (1 + Math.Pow(Math.E, (double) (-1 * input)))));
            // (input => { if (input < 0) { return input; } return Math.Min(10000000, input); });
            // (input => (float) ( 1 / (1 + Math.Pow(Math.E, (double) (-1 * input)))));
        ...
        private void SetRandConnections()
        {
            for (int i = 0; i < Connections.GetLongLength(0); i++)
            {
                ...
                {
                    Connections[i, j] = (float)(Globals.rand.NextDouble() * 2 - 1) * 5f;
                }
            }
        }
        ...
        #region BackProps
        internal void BackPropoldFirst(float[] desiredOut, float[] inputs, bool jgfdjhk)
        {
        ...
        internal void BackPropSecond(float[] inputs, float[] t)
        {
            float[] Ek = new float[Charges.Length];
